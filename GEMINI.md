...
- **Drone Scaling & Placement:** To prevent the background drone from overlapping and blocking text content on mobile devices (`<= 900px`), dynamically scale it down significantly (e.g., `scale(0.35)`) in JavaScript. Avoid relying solely on CSS `z-index` tricks to hide it behind text, as this often leads to stacking context bugs or makes the drone disappear entirely. Additionally, ensure the initial entry scale on PC remains moderate (around `2.5`) so it doesn't overwhelm the screen.

- **`display: contents` bug:** Never use `display: contents` on a container if any of its children rely on `position: absolute` for precise centering (e.g., `left: 50%`). Safari/WebKit browsers incorrectly calculate the containing block in this scenario, causing severe layout shifts (e.g., navigation bars jumping to the right side of the screen). Use standard flex layouts or leave the wrapper as `position: static` instead.
