...
# Debugging & Recovery
- **Revert-First Rule:** When a CSS/layout fix breaks the UI and the user reports it's wrong, IMMEDIATELY revert the affected files to the last known-good git commit (`git checkout <commit> -- <files>`) instead of making additional speculative forward-fixes. Only after confirming the revert restores the working state, make ONE carefully tested change at a time. Never chain more than one untested layout fix without user confirmation.
